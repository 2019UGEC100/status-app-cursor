...
export default async function OrgStatusPage({ params }: { params: Promise<{ orgId: string }> }) {
  ...
  const services = await db.service.findMany({ where: { orgId }, orderBy: { createdAt: "asc" } });
  const incidents = await db.incident.findMany({ where: { orgId }, orderBy: { createdAt: "desc" }, include: { updates: true } });
